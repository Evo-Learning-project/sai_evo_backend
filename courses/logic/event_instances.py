...
class ExercisePicker:
    def prefetch_exercises_from_rules(
        self, initial_qs, rules: List[EventTemplateRule]
    ) -> List[Exercise]:
        """
        ...
        ret = [
            e for e in initial_qs.filter(condition).with_prefetched_related_objects()
        ]
        return ret
    ...
    def get_exercises_from(
        self, template: EventTemplate, public_only=False, exclude_seen_in_practice=False
    ):
        """
        ...
        picked_exercises: List[Tuple[Exercise, EventTemplateRule]] = []
        ...
        if len([r for r in rules if r.rule_type == EventTemplateRule.ID_BASED]) > 0:
            # if there are ID-based rules, prefetch all exercises referred by those rules
            prefetched_exercises = self.prefetch_exercises_from_rules(exercises, rules)
        ...
        for rule in rules:
            ...
            if rule.rule_type == EventTemplateRule.ID_BASED:
                ...
                rule_picked_exercises = self.get_random_exercises_from_list(
                    [
                        e
                        for e in prefetched_exercises
                        # restrict to exercises referred by the rule
                        if e.pk in rule_ids
                        # avoid duplicates
                        and e.pk
                        not in [picked_e.pk for (picked_e, _) in picked_exercises]
                    ],
                    rule.amount,
                )
            else:
