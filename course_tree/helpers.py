...
class ThumbnailGenerationException(RuntimeError):
    pass
...
def get_file_thumbnail(file, mime_type: str):
    ...
    if base_type == "video":
        ...
        if not success:
            raise ThumbnailGenerationException(
                "videocap.read() failed for file " + file.name
            )
        success, encoded_frame = cv2.imencode(".jpeg", frame)
        if not success:
            raise ThumbnailGenerationException(
                "cv2.imencode() failed for file " + file.name
            )
